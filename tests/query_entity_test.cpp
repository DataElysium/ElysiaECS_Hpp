...
TEST(ElysiaQueryEntityHeader, EntityAsFirstArg) {
    World world;
...
TEST(ElysiaQueryEntityHeader, EntityAsLastArg) {
    World world;
...
TEST(ElysiaQueryEntityHeader, EntityWithFilter) {
    World world;
    struct Tag {};

    auto e1 = world.spawn().add(Value{1}).add(Tag{}).entity;
    world.spawn().add(Value{2});
