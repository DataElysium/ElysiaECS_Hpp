# Elysia ECS (Header-Only Version)

**Elysia ECS** is a high-performance, modern Entity-Component-System engine written in C++20. It combines the extreme performance of Structure of Arrays (SoA) chunk-iteration with an incredibly ergonomic, Bevy/Flecs-inspired Lambda API.
...
1. **Lambda Auto-Query**: The scheduler automatically builds queries based on your lambda signature.
2. **Resource Injection (`Res<T>`)**: Transparently request global resources directly in your system signature.
3. **Command Buffers**: Thread-safe, deferred world mutations (`cmd.spawn`, `cmd.despawn`, `cmd.add`, `cmd.remove`).
4. **DAG System Scheduling**: Use `.after("SystemName")` and `.before("SystemName")` to build a complex execution graph.
5. **High-Performance Bulk Iteration**: Need to squeeze every cycle out of the CPU? Bypass lambdas and iterate directly over SoA memory chunks using `BulkSystems` (see `docs/systems_api.md`).
...
```bash
# Configure the project (Uses Clang by default for C++20 Module/Parameter pack support)
xmake f -c 

# Build and run the Hello World example
xmake build ElysiaHeaderHelloWorld
xmake run ElysiaHeaderHelloWorld

# Build and run the Boids demo
xmake build ElysiaHeaderBoids
xmake run ElysiaHeaderBoids
```
