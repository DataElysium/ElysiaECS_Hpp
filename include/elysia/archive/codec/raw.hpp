...
struct RawVersionPatch { uint32_t id; uint16_t version; };
struct RawArchetypeHeader { uint32_t entity_count = 0; uint32_t component_count = 0; };
struct RawColumnHeader { uint64_t type_id = 0; uint32_t size = 0; uint32_t alignment = 0; char name[64] = {0}; };
...
Result<void> pack_world_raw(S& stream, World& world, bool unchecked = false) {
        ...
        for (size_t i = 0; i < world.archetype_count(); ++i) {
            ...
            for (const auto* info : arch->types()) {
                if (info->hooks.dtor || info->hooks.move) {
                    return Result<void>::err(ErrorCode::InvalidOperation, "Unsafe Raw Pack: Component '" + std::string(info->name) + "' has custom lifecycle hooks (non-POD).");
                }
            }
        }
    }

    RawWorldHeader h; h.archetype_count = static_cast<uint32_t>(world.archetype_count());
    if (auto r = stream.write_bytes(&h, sizeof(h)); r.is_err()) return r;

    const auto& records = world.index().records();
    std::vector<EntitySegment> segments; std::vector<RawVersionPatch> patches;
    for (size_t i = 0; i < records.size(); ++i) {
        if (records[i].active) {
            size_t start = i;
            while (i < records.size() && records[i].active) {
                if (records[i].version > 0) patches.push_back({static_cast<uint32_t>(i), records[i].version});
                i++;
            }
    ...
    if (!recycled.empty()) if (auto r = stream.write_bytes(recycled.data(), recycled.size() * sizeof(uint32_t)); r.is_err()) return r;
    if (!segments.empty()) if (auto r = stream.write_bytes(segments.data(), segments.size() * sizeof(EntitySegment)); r.is_err()) return r;
    if (!patches.empty()) if (auto r = stream.write_bytes(patches.data(), patches.size() * sizeof(RawVersionPatch)); r.is_err()) return r;

    for (size_t i = 0; i < world.archetype_count(); ++i) {
        auto* arch = world.graph().get_archetype(i); if (!arch) continue;
        RawArchetypeHeader ah; ah.entity_count = arch->count(); auto types = arch->types(); ah.component_count = static_cast<uint32_t>(types.size());
        if (auto r = stream.write_bytes(&ah, sizeof(ah)); r.is_err()) return r;
        for (const auto* info : types) {
            RawColumnHeader ch; ch.type_id = info->id; ch.size = static_cast<uint32_t>(info->size); ch.alignment = static_cast<uint32_t>(info->alignment);
        ...
        for (auto& ck : arch->table().chunks()) if (auto r = stream.write_bytes(&ck->entity(0), ck->count() * sizeof(Entity)); r.is_err()) return r;
...
Result<void> stream_into_chunks(S& stream, Archetype<>& storage, size_t start_row, size_t total_count, size_t elem_size, auto writer_fn) {
    size_t processed = 0; size_t current_row = start_row;
    while (processed < total_count) {
        ...
        processed += batch; current_row += batch;
    }
    return Result<void>::ok();
}
...
Result<void> unpack_world_raw(S& stream, World& world) {
    ...
    RawSkeletonHeader sh; auto& records = world.index().records();
    if (h.version == 2) {
        struct RawSkeletonHeaderV2 { uint32_t next_free_id; uint32_t record_capacity; uint32_t segment_count; uint32_t nonzero_version_count; } sh2;
        if (auto r = stream.read_bytes(&sh2, sizeof(sh2)); r.is_err()) return r;
        sh.next_new_id = sh2.record_capacity; sh.record_capacity = sh2.record_capacity; sh.recycled_count = 0; sh.segment_count = sh2.segment_count; sh.nonzero_version_count = sh2.nonzero_version_count;
    } else {
        if (auto r = stream.read_bytes(&sh, sizeof(sh)); r.is_err()) return r;
    }

    records.assign(sh.record_capacity, EntityRecord{}); world.index().set_next_new_id(sh.next_new_id);
    if (sh.recycled_count > 0) { world.index().recycled_ids().resize(sh.recycled_count); if (auto r = stream.read_bytes(world.index().recycled_ids().data(), sh.recycled_count * sizeof(uint32_t)); r.is_err()) return r; }
    if (sh.segment_count > 0) {
        ...
        for (const auto& seg : segments) for (uint32_t i = 0; i < seg.count; ++i) { size_t id = static_cast<size_t>(seg.start + i); if (id < records.size()) records[id].active = true; }
    }
    if (sh.nonzero_version_count > 0) {
        ...
        for (const auto& patch : patches) if (patch.id < records.size()) records[patch.id].version = patch.version;
    }

    for (uint32_t i = 0; i < h.archetype_count; ++i) {
        RawArchetypeHeader ah; if (auto r = stream.read_bytes(&ah, sizeof(ah)); r.is_err()) return r;
        ...
        for (uint32_t c = 0; c < ah.component_count; ++c) {
            ...
            if (!info) info = world.graph().registry().register_opaque(cols[c].type_id, cols[c].name, cols[c].size, cols[c].alignment);
            target_types.push_back(info);
        }
        auto* arch = world.graph().get_or_create(target_types);
        using ArchType = Archetype<DefaultConfig>; auto* storage = static_cast<ArchType*>(arch);
        if (ah.entity_count == 0) continue;
        size_t start_row = storage->modify().push_batch_raw(ah.entity_count);
        Result<void> res = stream_into_chunks(stream, *storage, start_row, ah.entity_count, sizeof(Entity), [](Chunk* c, size_t idx) { return &c->entity(idx); });
        ...
        for (uint32_t k = 0; k < ah.entity_count; ++k) {
            ...
            if (e.id() < records.size() && records[e.id()].active && records[e.id()].version == e.version()) {
                records[e.id()].archetype = storage; records[e.id()].row = static_cast<uint32_t>(start_row + k);
            }
        }
        for (uint32_t c = 0; c < ah.component_count; ++c) {
...
public:
    static std::vector<char> export_archetype(Archetype<>& arch, const std::vector<const ComponentFactory*>& active) {
        std::vector<char> buf; VectorWriter writer{buf};
        for (auto& ck : arch.table().chunks()) writer.write_bytes(&ck->entity(0), ck->count() * sizeof(Entity));
        for (auto* fac : active) {
            auto col_idx = arch.get_column_index(fac->type_id);
            if (!col_idx) continue;
            for (auto& ck : arch.table().chunks()) writer.write_bytes(ck->component(*col_idx, 0), ck->count() * fac->size);
        }
        return buf;
    }

    static Result<void> load(World& world, CommandBuffer& cmd, const std::vector<const ComponentFactory*>& active, std::span<const char> bytes, uint32_t entity_count, const std::unordered_map<uint64_t, Entity>& id_map) {
