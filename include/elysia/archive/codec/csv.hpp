...
inline Result<void> load_from_csv_flattened(World& world, CommandBuffer& cmd, const SnapshotRegistry& reg, const std::string& csv, const std::unordered_map<uint64_t, Entity>& id_map) {
    ...
    while (std::getline(ss, line)) {
        ...
        for (const auto& [comp, fields] : to_build) {
            ...
            for (const auto& [fk, fv] : fields) {
                ...
                if (fv == "true" || fv == "false" || fv == "null") is_primitive = true;
                else if (!fv.empty() && (fv.front() == '{' || fv.front() == '[')) is_primitive = true;
                else if (!fv.empty()) { char* end = nullptr; std::strtod(fv.c_str(), &end); is_primitive = (end && end != fv.c_str() && *end == '\0'); }
                json += "\""; json += fk; json += "\":";
                if (is_primitive) json += fv; else {
                    json += "\"";
                    for (char c : fv) { if (c == '"') json += "\\\""; else if (c == '\\') json += "\\\\"; else json += c; }
                    json += "\"";
                }
                first = false;
            }
            ...
            for (const auto& [fid, fac] : reg.factories()) {
                if (fac.key == comp) {
                    if (fac.generic) {
                        ...
                        if (g) { auto res = fac.generic->from_generic_cmd(cmd, target, *g); if (res.is_err()) return res; }
                        else return Result<void>::err(ErrorCode::InternalError, "Failed to parse CSV JSON");
                    }
                    break;
                }
            }
...
inline Result<void> load_from_csv_flattened(World& world, CommandBuffer& cmd, const SnapshotRegistry& reg, const std::string& csv, const IDRemapRegistry& id_reg, const IDMapper& mapper) {
    std::stringstream ss(csv); std::string line; if (!std::getline(ss, line)) return Result<void>::ok();
    auto headers = detail::split_csv_line(line);
    while (std::getline(ss, line)) {
        if (line.empty()) continue;
                    ...
                    if (fac.generic) {
                        ...
                        if (g) {
                            ...
                            if (const auto* hook = id_reg.get_hook(fac.type_id)) {
                                void* ptr = cmd.last_payload(); if (ptr) (*hook)(ptr, mapper);
                            }
                        } else return Result<void>::err(ErrorCode::InternalError, "Failed to parse CSV JSON");
                    }
                    break;
