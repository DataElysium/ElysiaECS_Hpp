...
template <size_t N = 4> struct SignatureBuffer {
    using StorageType = std::conditional_t<(N > 0), std::array<uint64_t, N>, std::vector<uint64_t>>;
    ...
    void set(uint32_t bit_idx) {
        ...
        if constexpr (N == 0) {
            if (chunk_idx >= data.size()) data.resize(chunk_idx + 1, 0);
        }
    ...
    [[nodiscard]] bool test(uint32_t bit_idx) const noexcept {
        const uint32_t chunk_idx = bit_idx >> 6;
        if (chunk_idx >= data.size()) return false;
    ...
    [[nodiscard]] size_t hash_value() const noexcept {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            h ^= static_cast<size_t>(val) + static_cast<size_t>(0x9e3779b97f4a7c15ULL) + (h << 6) + (h >> 2);
        }
        return h;
    }
};
...
template <size_t N>
[[nodiscard]] inline bool match_all(const SignatureBuffer<N>& arch, const SignatureBuffer<N>& with) noexcept {
    const size_t check_len = std::min(arch.data.size(), with.data.size());
    for (size_t i = 0; i < check_len; ++i) {
        if ((arch.data[i] & with.data[i]) != with.data[i]) return false;
    }
    for (size_t i = check_len; i < with.data.size(); ++i) {
        if (with.data[i] != 0) return false;
    }
    return true;
}

template <size_t N>
[[nodiscard]] inline bool intersects(const SignatureBuffer<N>& a, const SignatureBuffer<N>& b) noexcept {
...
template <size_t N> struct hash<elysia::SignatureBuffer<N>> {
    size_t operator()(const elysia::SignatureBuffer<N>& sig) const noexcept { return sig.hash_value(); }
};
} // namespace std
