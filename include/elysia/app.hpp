...
public:
    ...
    void add_startup_system(std::string name, Func&& func) {
        startup_scheduler_.system(std::move(name)).run(std::forward<Func>(func)).build(startup_scheduler_);
    }
    ...
    void update() {
        if (!is_started_) init_serial();
        if (use_parallel_) parallel_executor_->run(&world_);
        else executor_->run(&world_);
    }

private:
    World world_;
    Scheduler scheduler_;
    Scheduler startup_scheduler_;
